...
const SUBGROUP_RULES = [
  ...
  {
    match: (e) => /heart/i.test(e.k + e.n) && /heart/.test(e.c),
    tag: "30_heart",
  },
  { match: (e) => /emotion/i.test(e.k + e.n), tag: "40_emotion" },
  { match: (e) => /hand-fingers-open/i.test(e.k + e.n), tag: "50_hand-open" },
  {
    match: (e) => /hand-fingers-partial/i.test(e.k + e.n),
    tag: "51_hand-partial",
  },
  {
    match: (e) => /hand-fingers-closed/i.test(e.k + e.n),
    tag: "52_hand-closed",
  },
  {
    match: (e) => /hand-single-finger/i.test(e.k + e.n),
    tag: "53_hand-single",
  },
  ...
  {
    match: (e) => /person-activity/i.test(e.k + e.n),
    tag: "73_person-activity",
  },
  ...
  {
    match: (e) => /animal-amphibian|animal-reptile/i.test(e.k + e.n),
    tag: "82_reptile",
  },
  { match: (e) => /animal-marine/i.test(e.k + e.n), tag: "83_marine" },
  { match: (e) => /animal-bug/i.test(e.k + e.n), tag: "84_bug" },
  {
    match: (e) => /plant-flower|plant-other/i.test(e.k + e.n),
    tag: "85_plant",
  },
  {
    match: (e) =>
      /food-fruit|food-vegetable|food-prepared|food-asian|food-marine|food-sweet/i.test(
        e.k + e.n,
      ),
    tag: "90_food",
  },
  ...
  {
    match: (e) =>
      /place-map|place-geographic|place-building|place-religious|place-other|place-park/i.test(
        e.k + e.n,
      ),
    tag: "a0_place",
  },
  {
    match: (e) =>
      /transport-ground|transport-water|transport-air/i.test(e.k + e.n),
    tag: "a1_transport",
  },
  ...
  {
    match: (e) =>
      /phone|computer|light|tool|medical|household|office|lock|key|tool-other/i.test(
        e.k + e.n,
      ),
    tag: "b3_object",
  },
  ...
  {
    match: (e) => /other-symbol|alphanum/i.test(e.k + e.n),
    tag: "c6_other-symbol",
  },
...
const reordered = source.replace(
  ...
  (block) => {
    ...
    const indent = "      ";
    const rebuilt = unique
      .map((e) => `${indent}{ c: "${e.c}", n: "${e.n}", k: "${e.k}" },`)
      .join("\n");

    const totalUniqueChars = new Set(unique.map((e) => e.c)).size;
    console.log(
      `[${id}] in=${entries.length} dedupExact=${unique.length} uniqueChars=${totalUniqueChars}`,
    );
