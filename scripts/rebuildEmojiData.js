...
for (const row of rows) {
  ...
  if (!id) continue;
  const subgroupMatch = row.description.match(
    /\b([a-z][a-z0-9-]*-[a-z]+)\b\s*$/,
  );
  ...
  const name = keywords[0]
    ? keywords
        .slice(
          0,
          keywords.indexOf(
            row.description.match(/\b[a-z]+-[a-z]+/)?.[0] || "",
          ) > 0
            ? keywords.indexOf(row.description.match(/\b[a-z]+-[a-z]+/)?.[0])
            : 3,
        )
        .join(" ")
    : row.description;
...
for (const cat of CATEGORIES_OUT) {
  ...
  for (const e of cat.emojis) {
    ...
    out.push(
      `      { c: "${e.c}", n: ${JSON.stringify(e.n)}, k: ${JSON.stringify(e.k)}${tFlag} },`,
    );
  }
  out.push("    ],");
  out.push("  },");
