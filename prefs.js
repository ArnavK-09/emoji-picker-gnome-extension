...
class PreferencesBuilder {
  ...
  buildShortcutsGroup() {
    ...
    const shortcutButton = new Gtk.Button({ has_frame: false, valign: Gtk.Align.CENTER });
    ...
    const IGNORED_KEYVALS = new Set([
      ...
      Gdk.KEY_ISO_Level3_Latch, Gdk.KEY_ISO_Level5_Latch,
    ]);
  ...
  _findConflicts(accel) {
    ...
    const checkSource = (source) => {
      ...
      for (const schemaId of schemaIds) {
        ...
        let schema;
        try {
          schema = new Gio.Settings({ settings_schema: source.lookup(schemaId, true) });
        } catch (e) {
          continue;
        }
        for (const key of schema.list_keys()) {
          ...
          if (value.get_strv().includes(accel)) conflicts.push(`${schemaId}: ${key}`);
        }
      }
    };
    ...
    if (extensionsRoot) {
      const enumChildren = extensionsRoot.enumerate_children('standard::name,standard::type', Gio.FileQueryInfoFlags.NONE, null);
      let info;
      while ((info = enumChildren.next_file(null))) {
        if (info.get_file_type() !== Gio.FileType.DIRECTORY) continue;
        const schemasDir = extensionsRoot.get_child(info.get_name()).get_child('schemas');
        if (!schemasDir.query_exists(null)) continue;
        const source = Gio.SettingsSchemaSource.new_from_directory(schemasDir.get_path(), defaultSource, false);
        checkSource(source);
      }
  ...
  buildBehaviorGroup() {
    ...
    for (const category of CATEGORIES) categoryOptions.push({ id: category.id, label: category.label });

    group.add(this._createComboRow('Default category', SETTING.DEFAULT_CATEGORY, categoryOptions));
    ...
    this._settings.bind(SETTING.RECENT_COUNT, recentCount, 'value', Gio.SettingsBindFlags.DEFAULT);
    ...
    group.add(this._createComboRow('Skin tone', SETTING.SKIN_TONE, toneOptions));

    const pasteRow = new Adw.SwitchRow({
      title: 'Paste on select',
      subtitle: 'Type the selected emoji into the focused field (Shift+Insert).',
    });
    this._settings.bind(SETTING.PASTE_ON_SELECT, pasteRow, 'active', Gio.SettingsBindFlags.DEFAULT);
    group.add(pasteRow);
